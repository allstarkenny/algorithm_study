...
def solution(n):
    ...
    for i in range(1, n):
        ...
        for d in range(len(distance)):
            if distance[d] * 2 > n:
                battery = (d+1) + n - distance[d]
                print('case 1 {}'.format(str(battery)))
                if minimum_battery > battery:
                    minimum_battery = battery
                # if minimum_battery < i:
                #     print('return {}'.format(str(minimum_battery)))
                #     return minimum_battery
            elif distance[d] * 2 == n:
                battery = (d + 1)
                print('case 2 {}'.format(str(battery)))
