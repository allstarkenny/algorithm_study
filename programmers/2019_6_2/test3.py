...
# 와 같은 순서로 말을 하게 되며,
# 3번 사람이 자신의 세 번째 차례에 말한 tank라는 단어가 1번 사람이
...
def solution(n, words):
    ...
    for i in range(rotate_cnt):
        ...
        if len(words) < 1:
            break
        print(keyword, words)
        player_pick = search_keyword(keyword, words, used_words)
        print(player_pick)
        if player_pick:
            player[i%n].append(player_pick)
            keyword = player_pick[-1]
            words.remove(player_pick)
            used_words.append(player_pick)
        else:
            print([i % n + 1, len(player[i % n]) + 1])
            return [i % n + 1, len(player[i % n]) + 1]

    print([0, 0])
    return [0, 0]

def search_keyword(keyword, words, used_words):
    for w in words:
        if w[0] == keyword and w not in used_words:
            return w

    return False

arr1 = [3, 5, 2]
arr2 = [['tank', 'kick', 'know', 'wheel', 'land', 'dream', 'mother', 'robot', 'tank'],
